...
- 检查 Jetson/RK3588 开发环境和依赖
- 检测摄像头、显示器、USB、磁盘、网卡、I2C/SPI 外设
- 浏览远程路径，上传/下载单文件，新建和删除远程路径
- 查看、启动、停止、重启 systemd 服务并跟踪服务日志
- 生成 TensorRT engine 转换命令和 RKNN 部署模板
- 保存多设备 SSH 和路径档案
- 导出 Markdown 诊断报告
...
- 左侧导航栏：可滚动入口，快速切换代理、项目传输、运行控制、进程管理、日志查看、网络诊断、环境检查、外设检测、文件管理、服务管理、模型部署、设备档案、诊断报告、设备状态、显示设置和命令参考。
...
- `环境检查`：检查 OS、Python、构建工具、OpenCV、FFmpeg、Jetson CUDA/TensorRT、RK3588/RKNPU。
