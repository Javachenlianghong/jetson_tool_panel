...
- `SSH 工作台`：采用类似 FinalShell 的同屏布局，左侧是远端/本地目录树、文件列表和传输进度，右侧是持久 SSH shell；连接成功后会尝试刷新远端文件列表。文件表支持右键上传、下载、复制路径、打开本地位置，以及把远端目录发送到终端执行 `cd`。首次未知 host key 会自动接受并记录提示。认证优先使用系统 SSH key/agent，失败时弹窗输入密码，密码只保存在当前会话内。
